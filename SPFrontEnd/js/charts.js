...
stockapp.controller("stockController", function ($scope, $http) {
    ...
    $http({
        ...
        if (response) {
            for (var i = 0; i < response.length / 2; i++) {
                firstHalf[i] = response[i];
            }
            for (var i = response.length / 2; i < response.length; i++) {
                secondHalf[i - response.length / 2] = response[i];
            }
            $scope.stocks1 = firstHalf;
            $scope.stocks2 = secondHalf;
        }
    ...
    $http({
        method: 'GET',
        url: 'http://localhost:8080/v1/api/twittertrend'

    }).success(function (response1) {

        for (var j = 0; j < 11; j++) {
            resVal[j] = response1[j];
        }

        $scope.newsFeed = resVal;
    ...
    $scope.showDynamoGraph = function(cName){
        ...
        $http({
            ...
            for (var i = 0; i < response2.length; i++) {
                dataArr.push([response2[i].date, response2[i].value]);
            }
...
$.ajax({
    ...
    url: "http://localhost:8080/v1/api/stockhistory?company=INTU",
    type: 'GET',
    dataType: 'JSON',
    asynch: false,
    // data: JSON.stringify({
    ...
    success: function (data1, textStatus) {
        ...
        for (var i = 0; i < data1.length; i++) {
            dataArr.push([data1[i].date, data1[i].value]);
        }
...
function showDynamoGraph(cName){
    var urlCompanyName;
    if(cName == "CISCO"){
        urlCompanyName = "CSCO";
    }
...
function showGraph(dataArr) {
    ...
    if ($('#line-chart')[0]) {
        ...
        // var d1 = [[1,14], [2,15], [3,18], [4,16], [5,19], [6,17], [7,15], [8,16], [9,20], [10,16], [11,18] ,[12, 24]];
